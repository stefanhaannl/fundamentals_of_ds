...
def sentiment_tweets(df_tweetjes, S_DIR, Fname):
    ...
    geo_tweet = gpd.GeoDataFrame({"geometry": tweet_point})
...
def per_inhabitant(population_data_all, state_tweets):
    ...
    state_tweets_1mil = (state_tweets/state_pop) * 1000000

    return state_tweets_1mil
...
# Longitude latitudes staan nog verkeerd in de functions, hieronder staat het goed
    ...
    scaled_state = 'District of Columbia'#['Wyoming', 'Montana','North Dakota','South Dakota','Nebraska','Wisconsin','Iowa','Minnesota']    # State(s) that will be altered
...
#    state_pos_percent, state_pos_tweets = sentiment_tweets(df_tweetjes, S_DIR, Fname)
    state_tweets_1mil = per_inhabitant(population_data_all, state_tweets)
    
    # plot states
    plot_states(data, state_tweets_1mil, scalefactor, extra_long, extra_lat, scaled_state)


# Calculate percentage increase / decrease of tweet coverage per 1 mil inhabitants per state

#per_state_1mil_10k = state_tweets_1mil/np.nansum(state_tweets_1mil)*100
#(per_state_1mil_10k/per_state_1mil_20k-1)*100